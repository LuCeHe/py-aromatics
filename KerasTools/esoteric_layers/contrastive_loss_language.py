...
def contrastive_disorder(self, y_true, y_pred, ):
    ...
    contrastive_loss = - self.coef_disorder * tf.sigmoid(
...
def contrastive_random(self, y_pred):
    ...
    for i in range(self.n_random):
        ...
        contrastive_loss = - self.coef_random * tf.sigmoid(self.loss(random_words, y_pred))
        self.add_loss(contrastive_loss)
...
class ContrastiveLossLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, coef_disorder=.0, coef_random=.0, n_random=1,
        ...
        self.inaxis_shuffle = lambda s, p, t: None
