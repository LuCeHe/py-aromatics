...
from pyaromatics.keras_tools.esoteric_tasks.long_range_arena import create_pathfinder as pathfinder
...
_PATHFINER_TFDS_PATH = os.path.join(DATADIR, 'pathfinder', 'TFDS')
for d in [DATADIR]:
    os.makedirs(d, exist_ok=True)


def get_mnist_datasets(batch_size=256, normalize=False):
...
def get_cifar10_datasets(batch_size=256, normalize=False, flatten=True):
    ...
    def decode(x):
        ...
        if flatten:
            decoded['inputs'] = tf.reshape(decoded['inputs'], [-1])
...
def get_pathfinder_orig_datasets(batch_size=256, normalize=False, flatten=True):
